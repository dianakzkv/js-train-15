...
function calculateHypotenuse(a, b) {
	...
	if (typeof a !== "number" || typeof b !== "number") {
		console.log("a anb b must be numbers");
		return null;
	}
...
function getFractionalPart(num) {
	...
	if (typeof num !== "number") {
		console.log("num must be numbers");
		return null;
	}

	let integerPart = Math.floor(num);
	let fractionalPart = num - integerPart;
	let roundedFractionalPart = parseFloat(fractionalPart.toFixed(6));
...
function compareAndRound(num1, num2) {
	...
	let numRound = num1 > num2 ? Math.round(num1) : Math.round(num2);
	return numRound;
}
